import { useContext, createContext, useState, useEffect, useRef } from "react";
import NDK, {
  NDKEvent,
  NDKKind,
  NDKPrivateKeySigner,
  NDKRelay,
  NDKRelaySet,
  NDKSigner,
  NDKUser,
  NDKUserProfile,
  NostrEvent,
} from "@nostr-dev-kit/ndk";
import { generateSecretKey, getPublicKey } from "nostr-tools";

interface NDKContextProps {
  ndk: NDK;
  setSigner: (signer: NDKSigner) => void;
  removeSigner: () => void;
  fetchUserProfile: (pubkey: string) => Promise<NDKUserProfile | undefined>;
  fetchFollowingPubkeys: (pubkey: string) => Promise<string[]>;
  attemptDeleteEvent: (event: NostrEvent) => Promise<NDKEvent>;
  nip04Decrypt: (
    content: string,
    pubkey: string
  ) => Promise<string | undefined>;
  nip04Encrypt: (
    content: string,
    pubkey: string
  ) => Promise<string | undefined>;
  publishNostrEvent: (event: NostrEvent) => Promise<void>;
}

const NDKContext = createContext<NDKContextProps>({
  ndk: new NDK(),
  setSigner: (signer: NDKSigner) => {},
  removeSigner: () => {},
  fetchUserProfile: async (pubkey: string): Promise<NDKUserProfile> =>
    ({} as NDKUserProfile),
  fetchFollowingPubkeys: async (pubkey: string): Promise<string[]> => [],
  attemptDeleteEvent: async (event: NostrEvent): Promise<NDKEvent> =>
    ({} as NDKEvent),
  nip04Decrypt: async (content: string, pubkey: string) => undefined,
  nip04Encrypt: async (content: string, pubkey: string) => undefined,
  publishNostrEvent: async (event: NostrEvent) => new Promise(() => {}),
});

const defaultRelays = [
  "wss://nostr.mutinywallet.com/",
  "wss://relay.damus.io",
  "wss://relay.snort.social",
  "wss://relay.primal.net",
  "wss://bitcoiner.social",
  "wss://relay.satoshidnc.com",
  "wss://nos.lol",
];

export const initPrivkey = () => {
  const local = localStorage.getItem("privkey");
  let privkey: Uint8Array;
  if (local) {
    privkey = new Uint8Array(Buffer.from(local, "hex"));
  } else {
    privkey = generateSecretKey();
    localStorage.setItem("privkey", Buffer.from(privkey).toString("hex"));
  }
  console.log("privkey:", Buffer.from(privkey).toString("hex"));
  return { privkey, pubkey: getPublicKey(privkey) };
};

export const NDKProvider = ({ children }: any) => {
  const ndk = useRef(new NDK({ explicitRelayUrls: defaultRelays }));

  useEffect(() => {
    ndk.current
      .connect()
      .then(() => {
        console.log("Connected to NDK");
      })
      .catch((e) => {
        console.error(e);
      });
  });

  const setSigner = (signer: NDKSigner) => {
    ndk.current.signer = signer;
    ndk.current.assertSigner();
  };

  const removeSigner = () => {
    ndk.current.signer = undefined;
  };

  const fetchUserProfile = async (pubkey: string) => {
    const user = ndk.current.getUser({ pubkey });
    await user.fetchProfile();
    return user.profile;
  };

  const fetchFollowingPubkeys = async (pubkey: string) => {
    const user = ndk.current.getUser({ pubkey });
    const following = await user.follows({ closeOnEose: false });
    return Array.from(following).map((user) => user.pubkey);
  };

  const attemptDeleteEvent = async (event: NostrEvent) => {
    const eventToDelete = new NDKEvent(ndk.current, event);
    const deleted = await eventToDelete.delete("Deleted by user", true);
    console.log(deleted.rawEvent());
    return deleted;
  };

  const nip04Decrypt = async (content: string, pubkey: string) => {
    return ndk.current.signer?.encrypt(new NDKUser({ pubkey }), content);
  };

  const nip04Encrypt = async (content: string, pubkey: string) => {
    return ndk.current.signer?.decrypt(new NDKUser({ pubkey }), content);
  };

  const publishNostrEvent = async (event: NostrEvent, relays?: string[]) => {
    ndk.current.assertSigner();

    const e = new NDKEvent(ndk.current, event);

    if (relays) {
      const relaySet = new NDKRelaySet(
        new Set(relays.map((r) => new NDKRelay(r))),
        ndk.current
      );
      for await (const r of relaySet.relays) {
        await r.connect();
      }
      await e.publish(relaySet);
    } else {
      await e.publish();
    }
  };

  const contextValue: NDKContextProps = {
    ndk: ndk.current,
    setSigner,
    removeSigner,
    fetchUserProfile,
    fetchFollowingPubkeys,
    attemptDeleteEvent,
    nip04Decrypt,
    nip04Encrypt,
    publishNostrEvent,
  };

  return (
    <NDKContext.Provider value={contextValue}>{children}</NDKContext.Provider>
  );
};

export const useNdk = () => useContext(NDKContext);
